...
def format_pandas_table_as_xlsx(pandas_table,output_file_path,csv_file_path):
    ...
    pandas_table.to_excel(output_file_path, sheet_name=output_file_path.replace('xlsx',''),index=False,header=False)
    #open the excel file
...
def format_table_as_pdf(pandas_table,output_file_path):
    ...
    plt.axis('tight')
    plt.axis('off')
    table = plt.table(cellText=pandas_table.values, colLabels=pandas_table.columns, loc='center')
...
group.add_argument('-x','--xlsx',help='Convert the csv file to xlsx',action='store_const',const='xlsx',dest='format')
group.add_argument('-j','--json',help='Convert the csv file to json',action='store_const',const='json',dest='format')
group.add_argument('-t','--txt',help='Convert the csv file to txt',action='store_const',const='txt',dest='format')
group.add_argument('-htm','--html',help='Convert the csv file to html',action='store_const',const='html',dest='format')
group.add_argument('-xm','--xml',help='Convert the csv file to xml',action='store_const',const='xml',dest='format')
group.add_argument('-s','--sql',help='Convert the csv file to sql',action='store_const',const='sql',dest='format')
group.add_argument('-pdf','--pdf',help='Convert the csv file to pdf',action='store_const',const='pdf',dest='format')
group.add_argument('-m','--md',help='Convert the csv file to markdown',action='store_const',const='md',dest='format')
group.add_argument('-i','--image',help='Convert the csv file to an image',action='store_const',const='image',dest='format')
...
if __name__ == '__main__':
    ...
    if args.format:
        if args.format == 'html':
            ...
            output = format_pandas_table_as_xlsx(pandas_table,args.output,csv_file_path=args.path)
            ...
            output = format_pandas_table_as_markdown(pandas_table,delimiter=recognize_delimiter(args.path))
        elif args.format == 'txt':
            output = format_pandas_table_as_txt(pandas_table)
        elif args.format == 'image':
            output = format_pandas_table_as_image(pandas_table)
        elif args.format == 'csv':
            output = format_pandas_table_as_csv(pandas_table)
        else:
            print(args.format)
            
        if args.output and args.format != 'xlsx' and args.format != 'pdf' and args.format != 'image':
            ...
            if args.format != 'xlsx' and args.format != 'pdf' and args.format != 'image':
